...
func (p *MasterPlaylist) CalcMinVersion() (ver uint8, reason string, err error) {
	ver = minVer
	...
	for _, variant := range p.Variants {
		for _, alt := range variant.Alternatives {
			if strings.HasPrefix(alt.InstreamId, "SERVICE") {
				ver = 7
				reason = "SERVICE value for the INSTREAM-ID attribute of the EXT-X-MEDIA"
				break
			}
		}
	}
	...
	for _, define := range p.Defines {
		if define.Type == QUERYPARAM {
			ver = 11
			reason = "EXT-X-DEFINE tag with a QUERYPARAM attribute"
		}
	}
...
// required to support the playlist according to the [HLS Prococcol Version Compatibility].
// The reason is a human-readable string explaining why the version is required.
func (p *MediaPlaylist) CalcMinVersion() (ver uint8, reason string, err error) {
	ver = minVer
	reason = "minimal version supported by this library"
	...
	for _, segment := range p.Segments {
		if segment.Limit != 0 {
			ver = 4
			reason = "EXT-X-BYTERANGE tag"
			break
		}
	}
	if p.Iframe {
		ver = 4
		reason = "EXT-X-I-FRAMES-ONLY tag"
	}

	if p.Key != nil {
		if p.Key.Method == "SAMPLE-AES" || p.Key.Keyformat != "" || p.Key.Keyformatversions != "" {
			ver = 5
			reason = "EXT-X-KEY tag with a METHOD of SAMPLE-AES, KEYFORMAT or KEYFORMATVERSIONS attributes"
		}
	}
	if p.Map != nil {
		ver = 5
		reason = "EXT-X-MAP tag"
	}

	for _, segment := range p.Segments {
		if segment.Key != nil {
			if segment.Key.Method == "SAMPLE-AES" || segment.Key.Keyformat != "" ||
				segment.Key.Keyformatversions != "" {
				ver = 5
				reason = "EXT-X-KEY tag with a METHOD of SAMPLE-AES, KEYFORMAT or KEYFORMATVERSIONS attributes"
				break
			}
	...
	if p.Map != nil && !p.Iframe {
		ver = 6
		reason = "EXT-X-MAP tag in a Media Playlist that does not contain EXT-X-I-FRAMES-ONLY"
	}

	for _, segment := range p.Segments {
		if segment.Map != nil && !p.Iframe {
			ver = 6
			reason = "EXT-X-MAP tag in a Media Playlist that does not contain EXT-X-I-FRAMES-ONLY"
		}
	}
